...
const NewArrivals=()=>{
    ...
    const getData=()=>{
      setIsLoading(true)
        axios.get(`https://shy-plum-greyhound-cape.cyclic.app/products?_page=${page}&_limit=20`)
        .then((res)=>{
            setDatas(res.data);
            setIsLoading(false);
     ...
     <SimpleGrid columns={{base:2,sm:2,md:3,lg:4}} spacing={4} >
        ...
        >
         {el.name}
           ...
           <Box textAlign='left'>
            <Text  fontWeight='600' fontSize='large'>Rs{el.price}</Text>
            <Box display='flex' gap='3px'> 
              {/* <Text textAlign='left' as='del' fontSize='xs' color='gray'>Rs{el.CsSalesPrice
} </Text> */}
